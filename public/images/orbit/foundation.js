...
(function (a) {
  ...
  a.fn.reveal = function (b) {
    ...
    return this.each(function () {
      ...
      function k() {
        ...
        if (!h) {
          ...
          if (b.animation == "fadeAndPop") {
            ...
            l.delay(b.animationSpeed / 2).animate(
              { top: a(document).scrollTop() + g + "px", opacity: 1 },
              b.animationSpeed,
              j
            );
          }
      ...
      function f() {
        if (!h) {
          ...
          if (b.animation == "fadeAndPop") {
            ...
            l.animate(
              { top: a(document).scrollTop() - i + "px", opacity: 0 },
              b.animationSpeed / 2,
              function () {
      ...
      var d = a("." + b.dismissModalClass).bind(
        "click.modalEvent",
        function () {
...
(function (b) {
  var a = {
    ...
    init: function (f, e) {
      ...
      this.startTimerAfterMouseLeave = b.proxy(
        this.startTimerAfterMouseLeave,
        this
      );
      this.clearClockMouseLeaveTimer = b.proxy(
      ...
      if (this.options.bulletThumbs) {
        ...
        if (c) {
          h.addClass("has-thumb").css({
            background:
              "url(" + this.options.bulletThumbLocation + c + ") no-repeat",
          });
        }
      }
...
jQuery(document).ready(function (b) {
  function a(c) {
    b("form.custom input:" + c).each(function () {
      var e = b(this).hide(),
        d = e.next("span.custom." + c);
      if (d.length === 0) {
        d = b('<span class="custom ' + c + '"></span>').insertAfter(e);
      }
      d.toggleClass("checked", e.is(":checked"));
    });
  }
  a("checkbox");
  a("radio");
  b("form.custom select").each(function () {
    var e = b(this),
      g = e.next("div.custom.dropdown"),
      c = e.find("option"),
      d = 0,
      f;
    if (g.length === 0) {
      g = b(
        '<div class="custom dropdown"><a href="#" class="selector"></a><ul></ul></div>"'
      );
      c.each(function () {
...
(function (b) {
  ...
  b(document).on("click", "form.custom span.custom.checkbox", function (d) {
    d.preventDefault();
    d.stopPropagation();
    a(b(this));
  });
  b(document).on("click", "form.custom span.custom.radio", function (d) {
    d.preventDefault();
    d.stopPropagation();
    c(b(this));
  });
  b(document).on("click", "form.custom label", function (e) {
    var d = b("#" + b(this).attr("for")),
      g,
      f;
    if (d.length !== 0) {
  ...
  b(document).on("click", "form.custom div.custom.dropdown li", function (g) {
    var h = b(this),
...
(function (j, i, l) {
  ...
  if (k && h) {
    ...
    l.fn.placeholder = function () {
      return this.filter((k ? "textarea" : ":input") + "[placeholder]")
        .bind("focus.placeholder", o)
        .bind("blur.placeholder", m)
    ...
    if (a.val() === a.attr("placeholder") && a.hasClass("placeholder")) {
      if (a.data("placeholder-password")) {
        a.hide()
          ...
          .show()
          .focus()
          .attr("id", a.removeAttr("id").data("placeholder-id"));
      } else {
      ...
      if (b.is(":password")) {
        if (!b.data("placeholder-textinput")) {
          ...
          a.removeAttr("name")
            ...
            .data("placeholder-id", c)
            .bind("focus.placeholder", o);
          b.data("placeholder-textinput", a)
            .data("placeholder-id", c)
            .before(a);
        }
